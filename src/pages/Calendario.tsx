...
import { format, startOfMonth, endOfMonth, startOfWeek, endOfWeek, addDays, addMonths, subMonths, isSameMonth, isSameDay, addWeeks, subWeeks, startOfDay } from 'date-fns';
...
export default function Calendario() {
  ...
  const getOrdenesDelDia = (date: Date) => {
    return ordenesConCita.filter(o => o.fechaCita && isSameDay(o.fechaCita, date));
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <>
            ...
            <div className="grid grid-cols-7">
              {calendarDays.map((d, i) => {
                const eventos = getOrdenesDelDia(d);
                  ...
                  >
                    ...
                    <div className="mt-1 space-y-0.5">
                      {eventos.slice(0, 3).map(e => (
                        ...
                        >
                          {formatHora(e.fechaCita)} {e.clienteNombre.split(' ')[0]}
                        </div>
                      ))}
                      {eventos.length > 3 && (
                    ...
                    {eventos
                      ...
                      .map(e => (
                        <div
                          ...
                          style={{ backgroundColor: getTecnicoColor(e.tecnicoNombre || '') }}
                        >
                          <div className="font-medium">{formatHora(e.fechaCita)}</div>
        ...
        {vista === 'dia' && (
          <div className="p-4">
            {getOrdenesDelDia(currentDate).length === 0 ? (
                  ...
                  .map(e => (
                    ...
                    >
                      <div className="text-center min-w-[50px]">
                        <div className="text-lg font-bold text-[#0f3460]">{formatHora(e.fechaCita)}</div>
                      </div>
                      <div className="h-10 w-1 rounded-full" style={{ backgroundColor: getTecnicoColor(e.tecnicoNombre || '') }} />
                      <div className="flex-1">
                        <p className="font-semibold text-gray-900">{e.clienteNombre}</p>
                        <p className="text-sm text-gray-600">{e.equipoTipo} · {e.equipoMarca}</p>
                        <p className="text-xs text-gray-500 mt-0.5">{faseLabel(e.fase)} · {e.tecnicoNombre || 'Sin técnico'}</p>
                      </div>
                    </div>
                  ))}
              </div>
