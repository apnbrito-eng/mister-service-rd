...
| 14 | **Eliminar orden** | ✓ | ✓ | cond | ✗ | ✗ | ✗ | UI: `puede('ordenesEliminar')` (Ordenes.tsx:536, EliminarOrdenButton.tsx:24). Rule: `esAdminOCoord()` (firestore.rules:369). Operaria: la UI deja, la rule rechaza. **Inconsistencia UI vs rule** — operaria ve botón pero recibe permission-denied. Sprint propio sugerido. |
...
1. **Eliminar orden (#14):** UI deja a operaria (default `ordenesEliminar=true`), rule la rechaza con `esAdminOCoord` solamente. → Sprint propio: ajustar UI para esconder botón a operaria O ampliar rule a `puede('ordenesEliminar')`.
