...
| 14 | **Eliminar orden** | ✓ | ✓ | cond | ✗ | ✗ | ✗ | UI: `puede('ordenesEliminar')` (Ordenes.tsx:536, EliminarOrdenButton.tsx:24). Rule: `isAuth() && userData().permisos.ordenesEliminar == true` (firestore.rules:378, SPRINT-128 R2 2026-05-10). **Alineado UI ↔ rule:** ambos consultan el granular `ordenesEliminar`. Operaria por **default** `=false` (heredado de TODO_FALSE) → NO ve botón. Si admin le activa el granular persona-por-persona desde el modal, ve botón Y la rule permite. Admin/coord siempre `=true` por TODO_TRUE. **Inconsistencia RESUELTA en SPRINT-128**. |
...
1. ~~**Eliminar orden (#14):** UI deja a operaria cuando `ordenesEliminar=true` (default es `false` heredado de TODO_FALSE — corrección 2026-05-10 SPRINT-128: la versión anterior de este doc decía erróneamente "default `=true`"), rule la rechaza con `esAdminOCoord` solamente.~~ **[RESUELTO en SPRINT-128 R2 el 2026-05-10]** — rule cambiada de `esAdminOCoord()` a `isAuth() && userData().permisos.ordenesEliminar == true` (firestore.rules:378). UI ↔ rule ahora alineadas vía el granular. `npm run deploy:rules` ejecutado en el mismo commit del sprint (ver `git log --grep SPRINT-128`); lock actualizado a `29247a9...`. Soft delete (`eliminada=true`) sigue funcionando para todos los roles oficina vía `allow update` — no afectado por el cambio.
