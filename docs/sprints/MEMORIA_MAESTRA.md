...
**🟠 AL TOPE — sprints separados del rediseño (encolados pasada 54 al cerrar A.1):**

- **`SPRINT-DISENO-A.2-EMOJIS`** — quitar emojis decorativos del JSX. ~2 chunks. Autónomo.
- **`SPRINT-DISENO-A.3-ESCALA-TIPOGRAFICA`** — 5 clases globales en `index.css` (no migra código). Autónomo.
- **`SPRINT-DISENO-A.4-GRADIENTES-SATURADOS`** — gradientes decorativos → color sólido (heros marketing intactos). Autónomo.
- **`SPRINT-DISENO-A.5-LINT-LIMPIEZA`** — cierra deuda de A.1 (7 archivos con warnings preexistentes) + completa find&replace incluyendo modificadores `hover:`/`focus:`/opacidad. Autónomo.
- **`SPRINT-DISENO-B-TECNICO-ACCESIBILIDAD`** — botones 48px en `/tecnico`, sin text-[10px]. [NO CERRAR sin QA Jorge en celular].
- **`SPRINT-DISENO-C-DASHBOARD-REDUCIDO`** — Dashboard 3 bloques + skeletons + microcopy dominicano. [NO CERRAR sin QA Jorge].
- **`SPRINT-DISENO-D-PUBLICAS-DOMINICANO`** — HomePage testimonios + CTA dominicano + empty states. [NO CERRAR sin QA Jorge].
