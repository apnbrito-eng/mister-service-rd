...
**Prioridad:** 🟠 ALTA (la apariencia del software es la cara del negocio). **Estado:** EN_EJECUCION pasada 54 (desbloqueado vía `OK: jorge 2026-05-30 opcion=B`, ejecutando **camino 2: solo FASE A.1**). El stub permanece en `BLOQUEOS.md` para forensia. Scope acotado pasada 54: unificar `#0f3460` → `primary` y `#1a5fa8` → `primary-medium` en los archivos hardcoded de `src/` (Tailwind arbitrary classes). Cero cambio visual de marca (los hex son idénticos a los tokens `primary`/`primary-medium`). FASES A.2/A.3/A.4/A.5 + B + C + D quedan como sprints separados (re-encolados al cierre de esta pasada para próximas).
