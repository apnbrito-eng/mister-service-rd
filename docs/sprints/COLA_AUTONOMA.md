...
## SPRINT-WA-NUMERO-RESPALDO-MANUAL — Selector admin del número de envío de WhatsApp (respaldo manual ante bloqueos de Meta)

**Prioridad:** ALTA (resiliencia: si Meta bloquea un número, poder seguir respondiendo a clientes). **Estado:** ⊘ MOVIDO A BLOQUEOS 2026-05-22 pasada 40 — toca `firestore.rules` + `api/whatsapp/send.ts` (sub-regla CLAUDE.md "Cambios a firestore.rules" + "Cambios a endpoints api/ públicos"). Cowork escribió que Jorge aprobó la dirección general (manual + admin + Configuración), pero el coordinator requiere OK formal con shape `OK: jorge YYYY-MM-DD ...` antes de tocar rules + decisión de scope (Fase 1 sola, o esperar Fase 2). Ver `docs/sprints/BLOQUEOS.md` para opciones A/B/C/D de desbloqueo. Stub histórico abajo. **Origen:** Jorge detectó que el inbox agrupa por cliente y la respuesta sale por el último número usado; quiere poder cambiar manualmente el número de envío si Meta bloquea uno, y a futuro un WABA de respaldo.
