...
**Prioridad:** MEDIA. **Estado:** ⊘ MOVIDO A BLOQUEOS el 2026-05-23 por coordinator autónomo (`trabaja`, pasada 42). Razón: toca `api/whatsapp/send.ts` (endpoint público) + `firestore.rules` (rule nueva para `config/whatsapp_respuestas_rapidas`). El sprint mismo marcaba "ESCALA". Ver `BLOQUEOS.md` para las 4 decisiones requeridas y el shape del `OK: jorge ...`. Conservado acá como stub para forensia. **NO procesar hasta que vuelva a la cola vía `procesa bloqueos`.**
...
**Touch-list (auditar antes):** `api/whatsapp/send.ts` (enviadoPor + prepend nombre agente), `src/pages/InboxConversacion.tsx` (pasar uid/nombre + UI respuestas rápidas + mostrar enviadoPor), `src/components/inbox/MensajeBubble.tsx` (mostrar enviadoPor), `firestore.rules` (`config/whatsapp_respuestas_rapidas`), `src/services/` (service respuestas rápidas + flag nombre-agente). **Reviewer obligatorio (endpoint + rules). Deploy rules antes de cerrar.**

**Criterio:** outbox guarda quién envió + el chat lo muestra; el cliente recibe el nombre del agente antepuesto (si el flag está activo); respuestas rápidas editables + insertables con "/". Typecheck + cazadores + lint PASS.
