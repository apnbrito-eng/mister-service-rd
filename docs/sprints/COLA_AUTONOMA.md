...
**Prioridad:** BAJA-MEDIA. **Estado:** 🟧 ESCALADO a `docs/sprints/BLOQUEOS.md` el 2026-05-21 noche pasada 35 por coordinator autónomo. **Motivo:** requiere endpoint público nuevo (`api/whatsapp/media-proxy.ts`) + `storage.rules` versionado (SPRINT-138 prerequisito) + decisión de negocio sobre dónde van las fotos en la orden. Sprint anticipaba este escalado ("Si re-subir requiere cambiar `storage.rules` → ESCALAR"). Detalles + 3 opciones de approach en BLOQUEOS.md.
