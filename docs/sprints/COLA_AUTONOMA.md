...
## SPRINT-DISENO-E-FUENTE-PLUS-JAKARTA — 🟡 EN PRODUCCIÓN AWAITING QA JORGE (hash `4ce1a57`, pasada 57)

**Prioridad:** 🟠 ALTA. **Estado:** código commiteado + pusheado en pasada 57 (2026-06-01). [NO CERRAR sin QA Jorge — afecta apariencia visible]. Hash: `4ce1a57`. **Plan QA:** hard-refresh `/`, `/admin/dashboard`, `/admin/ordenes`, `/tecnico`, `/admin/clientes`, `/admin/inbox`, `/agendar` — la tipografía debe verse visiblemente diferente (más personalidad, menos "Vite default"). Si OK → agregar `QA: jorge YYYY-MM-DD HH:MM DISENO-E PASS`.

**Archivos tocados:** `index.html` (link Google Fonts + preconnect), `tailwind.config.js` (fontFamily.sans), `src/index.css` (body + .pac-container), `src/components/clientes/MapaClientes.tsx` (popup HTML manual Leaflet). Sin tocar paleta brand/accent/primary, sin tocar print stylesheets.

---

## SPRINT-DISENO-E-FUENTE-PLUS-JAKARTA (stub histórico para forensia)

**Prioridad:** 🟠 ALTA (afecta identidad visual de toda la app; debe ir ANTES de los otros sprints de diseño porque cambia el ritmo tipográfico). **Estado:** ✅ COMPLETADO pasada 57 hash `4ce1a57`. **Autónomo (no toca rules/pagos/endpoints/migraciones).** **[NO CERRAR sin QA Jorge — afecta apariencia visible].**

**Origen:** leak interno del prompt de diseño de Claude (`asgeirtj/system_prompts_leaks/Anthropic/claude-design.md`), revisado por Cowork 2026-06-01. Cita verbatim del documento: *"Avoid overused font families (Inter, Roboto, Arial, Fraunces, system fonts)."* Decisión de Jorge 2026-06-01 vía Cowork con AskUserQuestion: **Plus Jakarta Sans** (recomendada por Cowork de 3 alternativas presentadas; el documento NO sugiere una específica, solo descarta las trilladas).

**Justificación de la elección Plus Jakarta Sans:**

- Sans-serif moderna, soporta español dominicano perfecto (ñ, tildes, mayúsculas acentuadas).
- Confiable + legible al mismo tiempo — funciona igual en admin denso (Dashboard, Órdenes) y landing comercial (HomePage).
- Personalidad sin ser distractora.
- Disponible gratis en Google Fonts, weights 200-800.

**Touch-list expandido (auditar antes de tocar):**

1. **`index.html`** — agregar `<link>` a Google Fonts ANTES del bundle React:
   ...
   Razón: `preconnect` reduce FOIT (flash of invisible text) en celulares lentos (la audiencia técnico de campo).

2. **`tailwind.config.js`** — actualizar `fontFamily.sans`:
   ...
   `Inter` se elimina del fallback chain. `system-ui` queda como fallback inmediato si Google Fonts falla.

3. **`src/index.css`** — verificar que no haya `font-family` hardcoded en clases globales. Si existe alguna referencia a `Inter`, quitarla (debería heredar del tailwind base).

4. **Auditoría side-effects:** grep `font-family|fontFamily|Inter` en `src/**/*.{ts,tsx,css}`:
   - Cualquier `font-family: 'Inter'` literal → eliminar (hereda del default).
   - Cualquier estilo print (`Cotizaciones.tsx`, `Facturas.tsx`) → mantener serifs si los usan, NO migrar print a Plus Jakarta (los PDFs/print suelen necesitar fuente distinta intencionalmente).
   - Cualquier comentario/doc que mencione Inter como decisión de diseño → actualizar la mención.

**Restricciones duras:**

- NO migrar `font-family` en print stylesheets (Cotizaciones, Facturas, Conduces).
- NO tocar `tailwind.config.js` brand/accent/primary palette — solo la sección `fontFamily`.
- NO mover otros tokens (radius, shadow) en este sprint.
- Cazadores P-001..P-025 PASS, typecheck PASS, lint clean, build PASS.

**Criterio de éxito:**

- `npm run build` PASS.
- Hard-refresh `/`, `/admin/dashboard`, `/admin/ordenes`, `/tecnico`, `/admin/clientes`, `/admin/inbox`, `/agendar`: la tipografía cambia visiblemente — más personalidad, menos "Vite default".
- Cualquier título largo (nombres de clientes con tildes, descripciones de orden con ñ) se ve con la misma fluidez que con Inter.
- `grep -i "inter" tailwind.config.js src/index.css` retorna 0 hits relacionados a fuente.

**[NO CERRAR sin QA Jorge — afecta apariencia visible.** QA: hard-refresh las pantallas listadas, confirmar que se ve "más cuidado" sin perder legibilidad. Si la diferencia con Inter es muy sutil en mobile, considerar subir 1 weight (font-semibold → font-bold en headers). Si OK → `QA: jorge YYYY-MM-DD HH:MM DISENO-E PASS`.**]**

---

## SPRINT-DISENO-F-TEXT-WRAP-PRETTY — ✅ COMPLETADO pasada 57 (hash `45b6b38`)

**Estado:** COMPLETADO autónomo (sin QA dedicado por spec). Hash: `45b6b38`. `text-wrap: pretty` agregado a `.text-h1`/`.text-h2`/`.text-body` en `src/index.css`. Progressive enhancement: Safari <17.4 y Chrome <114 caen al wrapping default sin romper.

---

## SPRINT-DISENO-F-TEXT-WRAP-PRETTY (stub histórico)

**Prioridad:** 🟢 BAJA (trivial, alta-ROI). **Estado:** ✅ COMPLETADO pasada 57 hash `45b6b38`. **Autónomo.**

**Origen:** leak prompt diseño Claude, cita verbatim: *"text-wrap: pretty, CSS grid and other advanced CSS effects are your friends!"*. Aplica especialmente al proyecto porque los datos reales tienen nombres de clientes largos con tildes ("María Mercedes Hernández Liriano"), descripciones de orden largas, direcciones dominicanas largas ("Calle Respaldo 12 #45, Ensanche La Fé").

**Touch-list:**

`src/index.css` — agregar `text-wrap: pretty` a las 3 clases tipográficas globales definidas en SPRINT-DISENO-A.3:

```css
.text-h1 { @apply text-3xl font-bold text-primary; text-wrap: pretty; }
.text-h2 { @apply text-xl font-semibold text-primary; text-wrap: pretty; }
.text-body { @apply text-base font-normal text-gray-800; text-wrap: pretty; }
```

`.text-caption` y `.text-micro` NO necesitan — son textos cortos.

**Restricciones:**

- NO cambiar el resto de las clases.
- Si el browser no soporta `text-wrap: pretty` (Safari <17.4, Chrome <114), la línea simplemente cae al wrapping default — no rompe. Es 100% progressive enhancement.

**Criterio de éxito:**

- Build PASS.
- Visualmente: cualquier título de >2 líneas (ej: "Reparación de lavadora Whirlpool 18 lbs LSM-1000" en una card) rompe con balance entre líneas en vez de dejar una huérfana de 1 palabra al final.

**No requiere QA dedicado** — el cambio es invisible hasta que aparece un título largo. El coordinator puede marcarlo COMPLETADO al cerrar si pasan los cazadores + build.

---

## SPRINT-DISENO-G-NO-INVENTAR-CONTENIDO — ✅ COMPLETADO pasada 57 (hash `6fab6e9`)

**Estado:** COMPLETADO autónomo (sin QA dedicado — solo edita docs/meta-proceso). Hash: `6fab6e9`. Sub-regla agregada a `CLAUDE.md` "Conventions & gotchas" + sub-regla espejo en `.claude/agents/coordinator.md` (sección "What NEVER to delegate") + sub-regla espejo en `.claude/agents/builder.md` (convención #11). `grep "No inventar contenido sin OK" CLAUDE.md` retorna 1 hit.

---

## SPRINT-DISENO-G-NO-INVENTAR-CONTENIDO (stub histórico)

**Prioridad:** 🟠 ALTA (regla del meta-flujo, protege futuras pasadas autónomas). **Estado:** ✅ COMPLETADO pasada 57 hash `6fab6e9`. **Autónomo (solo edita CLAUDE.md).**

**Origen:** leak prompt diseño Claude, cita verbatim: *"If you think additional sections, pages, copy, or content would improve the design, ask the user first rather than unilaterally adding it."* Y también: *"Never pad a design with placeholder text, dummy sections, or informational material just to fill space. Every element should earn its place. If a section feels empty, that's a design problem to solve with layout and composition — not by inventing content."*

**Justificación:** durante SPRINT-DISENO-D (pasada 56) el coordinator estuvo a punto de inventar testimonios con clientes ficticios. Jorge lo cazó a tiempo ("recuerda que el software no esta en uso aun") y rediseñamos a placeholder-oculto-en-prod. La regla previene la próxima ocurrencia.

**Touch-list:**

`CLAUDE.md` — agregar nueva sub-regla en la sección **"Conventions & gotchas"**, entre las existentes y antes de **"Modo autónomo (Cowork ↔ Coordinator)"**. Texto sugerido (adaptar si el coordinator encuentra mejor lugar):

```markdown
- **No inventar contenido sin OK explícito de Jorge.** Si el builder o el
  coordinator consideran que una página/sección/copy/testimonio/ilustración
  debería agregarse para "mejorar el diseño", el cambio NO se ejecuta —
  se ESCALA a BLOQUEOS con propuesta concreta + 2-3 opciones. Aplica a:
  HomePage (testimonios, casos de éxito, "sobre nosotros", FAQ),
  Dashboard (KPIs/cards/widgets nuevos), TecnicoVista (atajos/secciones
  nuevas), ServiciosPage. NO aplica a refactors visuales de contenido
  existente. Antiprecedente SPRINT-DISENO-D pasada 56 (2026-05-31): el
  coordinator iba a inventar 3 testimonios ficticios; Jorge lo cazó
  ("recuerda que el software no está en uso aún") y redirigimos a
  placeholder-oculto-en-prod en `src/config/testimoniosHomePage.ts`.
  Origen externo de la regla: leak prompt diseño Claude — "Never pad a
  design with placeholder text, dummy sections, or informational material
  just to fill space" + "ask the user first rather than unilaterally
  adding it".
```

**Restricciones:**

- NO tocar otras secciones de CLAUDE.md.
- Coordinator decide la posición exacta dentro de "Conventions & gotchas" (orden lógico con las gotchas existentes).
- Hacer sub-regla espejo en `.claude/agents/coordinator.md` y `.claude/agents/builder.md` si tienen sección equivalente.

**Criterio de éxito:**

- `grep "No inventar contenido sin OK" CLAUDE.md` retorna 1 hit.
- Build PASS (CLAUDE.md no es código, pero por costumbre).
- Cazadores 25/25 PASS (no se rompe nada).

**No requiere QA visual** — es documentación de meta-proceso. El coordinator puede cerrar COMPLETADO al hacer el commit.

---

## SPRINT-DISENO-H-BORDER-LEFT-A-BADGE — 🟡 EN PRODUCCIÓN AWAITING QA JORGE (hash `cc673b0`, pasada 57)

**Prioridad:** 🟡 MEDIA. **Estado:** código commiteado + pusheado en pasada 57. [NO CERRAR sin QA Jorge en celular real + admin]. Hash: `cc673b0`. **Plan QA:** hard-refresh `/tecnico` (celular) + `/admin/ordenes` + `/admin/agenda` + tablero Kanban — verificar que (a) el estado de cada card sigue identificable de un vistazo (badge legible arriba-derecha), (b) no se rompió ningún tap target del técnico, (c) la jerarquía visual no quedó "plana". Si OK → `QA: jorge YYYY-MM-DD HH:MM DISENO-H PASS`.

**Archivos tocados:** `OrdenCard.tsx`, `Ordenes.tsx`, `AgendaDia.tsx`, `OrdenesTablero.tsx` (cards: border-left fuera, badge arriba-derecha si no existía). `OrdenDetalle.tsx`, `OrdenDetailModal.tsx` (banners: preservados con tag `@safe-border-left-accent`). Sin tocar handlers/lógica/fase/estadoSimple.

---

## SPRINT-DISENO-H-BORDER-LEFT-A-BADGE (stub histórico)

**Prioridad:** 🟡 MEDIA (anti-patrón explícito del prompt diseño Claude). **Estado:** ✅ COMPLETADO pasada 57 hash `cc673b0`. **Autónomo (no toca lógica).** **[NO CERRAR sin QA Jorge en celular real + admin].**

**Origen:** leak prompt diseño Claude, cita verbatim: *"Avoiding containers using rounded corners with a left-border accent color"*. Sospecha alta en TecnicoVista (cards de orden con border-left de color según fase/estado) y Ordenes.tsx / OrdenesTablero / OrdenCard / AgendaDia (mismo patrón).

**Touch-list expandido (auditar antes de tocar):**

1. **Grep el anti-patrón:**

   ```bash
   grep -rn "border-l-4\|border-l-2\|border-l-\[" src/components/ordenes/ src/pages/TecnicoVista.tsx src/pages/Ordenes.tsx src/pages/AgendaDia.tsx
   ```

   Para cada match: identificar si el border-left codifica un estado (fase de orden, prioridad, urgencia) — si sí, es anti-patrón a reemplazar.

2. **Reemplazar por badge arriba a la derecha:**

   - Cards de orden en TecnicoVista (lista de citas del día): el color del estado pasa a un `<span class="badge bg-X text-white text-xs px-2 py-1 rounded">` arriba-derecha de la card.
   - Cards de orden en Ordenes.tsx / OrdenesTablero: mismo tratamiento.
   - OrdenCard reutilizable: si tiene el border-left, reemplazar en el componente (cascada automática).
   - AgendaDia: mismo tratamiento si aplica.

3. **NO tocar:**

   - Border-left que es **estructural** (separador entre 2 columnas, no codifica estado).
   - Border-left en alerts/banners de error (`border-l-4 border-red-500 bg-red-50`) — es patrón aceptado de alerta, no anti-patrón. Documentar con comentario `// @safe-border-left-accent: banner de alerta`.
   - Border-left que esté en componentes de print/PDF.

4. **Coordinación con SPRINT-DISENO-A.5-COLORES-ICONOS:** ya hubo trabajo de unificación de colores; este sprint NO debe re-introducir colores hardcoded — usar `bg-primary`, `bg-emerald-500`, etc.

**Restricciones duras:**

- La codificación visual del estado se preserva (no se pierde información), solo cambia de border-left a badge.
- Sin cambios a lógica de fase/estado/estadoSimple.
- TecnicoVista: los 11 handlers críticos intactos (verificado en SPRINT-DISENO-B).
- Cazadores 25/25 PASS, typecheck PASS, lint clean.

**Criterio de éxito:**

- `grep -rn "border-l-4 border-(red\|amber\|yellow\|orange\|emerald\|green\|blue\|primary)" src/` retorna 0 en cards de orden (banners/alerts pueden quedar con tag de excepción).
- Cards de orden muestran el estado con badge arriba-derecha.
- Visualmente: las cards se ven más "diseñadas" y menos "del 2020".

**[NO CERRAR sin QA Jorge].** QA: hard-refresh `/tecnico` en celular + `/admin/ordenes` + `/admin/agenda`. Verificar que (a) el estado de cada card sigue siendo identificable de un vistazo (badge legible), (b) no se rompió ningún tap target del técnico, (c) la jerarquía visual no quedó "plana". Si OK → `QA: jorge YYYY-MM-DD HH:MM DISENO-H PASS`.
...
## SPRINT-DISENO-I-DATA-SLOP-DASHBOARD-AUDIT (stub histórico)

**Prioridad:** 🟡 MEDIA (limpieza cognitiva del Dashboard). **Estado:** ⊘ FASE 1 COMPLETADA hash `1177ec2`, FASE 2 ESCALADA. **Autónomo hasta el inventario, después ESCALA a Jorge para marcar cuáles SÍ usa.**

**Origen:** leak prompt diseño Claude, cita verbatim: *"Avoid 'data slop' — unnecessary numbers or icons or stats that are not useful."* Tras SPRINT-DISENO-C (Dashboard reagrupado en 3 bloques + KPI hero), todavía existen muchos contadores secundarios. La pregunta es: ¿cuáles usa Jorge para decidir algo y cuáles son ruido?

**Plan en 2 fases:**

### Fase 1 (autónoma) — Inventario

Generar `docs/sprints/INVENTARIO_KPI_DASHBOARD_<fecha>.md` con tabla:

| Sección | KPI / Card / Contador | Archivo:línea | Fuente del dato | Costo de mantenimiento | Pregunta a Jorge |
|---|---|---|---|---|---|
| Hoy | Citas pendientes hoy | Dashboard.tsx:XYZ | onSnapshot `citas_por_confirmar where estado=='pendiente'` | bajo | ¿Lo miras todos los días? |
| Pipeline | Cotizaciones enviadas semana | ... | ... | ... | ... |
| Plata | Ingresos mes vs mes anterior | ... | ... | ... | ... |

Cubrir TODOS los KPIs visibles en `Dashboard.tsx` post-SPRINT-DISENO-C. NO incluir el KPI hero "Órdenes atrasadas" (ya validado por Jorge en opción C).

Para cada KPI también documentar:
- Si es un número que se ve diariamente (alta-frecuencia) o eventual.
- Si la decisión que dispara existe (ej: "si ingresos < X, pongo más ads") o no (decoración).
- Si está duplicado en otra sección.

### Fase 2 (escala a Jorge) — ESCALAR a BLOQUEOS

Subir el inventario completo a `BLOQUEOS.md` con prompt:

> **DISENO-I espera tu input.** Abrí `docs/sprints/INVENTARIO_KPI_DASHBOARD_<fecha>.md`. Marcá con "MANTENER", "QUITAR" o "MOVER A REPORTE SEPARADO" cada KPI. Tras eso, agregá `OK: jorge YYYY-MM-DD opcion=mantener-X-quitar-Y-mover-Z` en BLOQUEOS y corré `procesa bloqueos`.

### Fase 3 (autónoma post-OK)

Aplicar las decisiones: eliminar los marcados "QUITAR", mover los marcados "MOVER" a una nueva ruta `/admin/reportes/<scope>` (sprint follow-up auto-encolado si aplica).

**Restricciones:**

- NO eliminar KPIs sin input explícito de Jorge.
- Si Jorge marca "QUITAR" un KPI y otro componente depende de él (ej: alguna alerta del sidebar), ESCALAR de vuelta con propuesta.
- Cazadores 25/25 PASS en cada fase.

**Criterio de éxito Fase 1:** inventario completo + escalación clean.

**[NO CERRAR sin QA Jorge].** El sprint se considera COMPLETADO cuando las 3 fases terminaron + Jorge da PASS visual. Si Jorge tarda en marcar el inventario, el sprint queda en BLOQUEOS sin urgencia.
