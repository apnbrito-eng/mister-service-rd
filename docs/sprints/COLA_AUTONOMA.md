...
**Última actualización:** 2026-05-15 por coordinator autónomo (pasada 18, `trabaja`) — **2 sprints COMPLETADOS en serie:** (1) **SPRINT-PERSONAL-EDIT-UNIFY** — `GestionUsuarios.tsx` ahora importa `ROL_LABELS`/`ROL_COLORS`/`ROL_SELECT_ORDEN` desde `utils/personal.ts` (single source of truth desde SPRINT-142d) eliminando duplicación local. Dropdown del modal Editar Usuario muestra ahora las 5 opciones con acceso al sistema (incluye Coordinadora que faltaba — bug que llevó a Jorge a tener que usar el modal alternativo al crear cuentas QA del SPRINT-QA-USER). +14/-13 líneas. (2) **SPRINT-158d-FIX** — `EnviarFacturacionButton.tsx` optimistic UI: toast + `setSaving(false)` ahora se ejecutan inmediatamente después del `updateDoc` crítico; notifs a admin/coord viajan en IIFE `void` fire-and-forget. Confirmación al usuario en ≤2s en lugar de 3-30s (caso Wilainy QA E2E 2026-05-13). +15/-3 líneas. Cazadores 10/10 PASS post-cada cambio. Typecheck + lint PASS. Hashes pendientes del commit en curso. **Cola autónoma agotada** (SPRINT-QA-USER queda PENDIENTE-bloqueado por acción humana de Jorge: crear las 5 cuentas QA antes que coordinator pueda cerrar).
