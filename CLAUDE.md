...
- **No inventar contenido sin OK explícito de Jorge.** Si el builder o el coordinator consideran que una página/sección/copy/testimonio/ilustración debería agregarse para "mejorar el diseño", el cambio NO se ejecuta — se ESCALA a `BLOQUEOS.md` con propuesta concreta + 2-3 opciones para que Jorge decida. Aplica especialmente a: HomePage (testimonios, casos de éxito, "sobre nosotros", FAQ), Dashboard (KPIs/cards/widgets nuevos), TecnicoVista (atajos/secciones nuevas), ServiciosPage. NO aplica a refactors visuales de contenido ya existente (mover, reordenar, retipografiar). Antiprecedente SPRINT-DISENO-D pasada 56 (2026-05-31): el coordinator iba a inventar 3 testimonios ficticios para llenar la HomePage; Jorge lo cazó (*"recuerda que el software no está en uso aún"*) y redirigimos a placeholder-oculto-en-prod en `src/config/testimoniosHomePage.ts` con flag `activo: false`. Origen externo de la regla: leak prompt diseño Claude — *"Never pad a design with placeholder text, dummy sections, or informational material just to fill space"* + *"If you think additional sections, pages, copy, or content would improve the design, ask the user first rather than unilaterally adding it"*. Sub-regla espejo aplicada en `.claude/agents/coordinator.md` y `.claude/agents/builder.md`.
