...
- ~~**Gotcha — bug pre-existente en `notificaciones` (sprint propio en backlog).** El código escribe campo `destinatarioId` en `src/services/notificaciones.service.ts`, pero `firestore.rules:511,515` gatean read/update/delete por `userId == request.auth.uid`. Resultado: técnicos/operarias/secretarias que reciben notificaciones NO pueden marcarlas como leídas (la rule evalúa contra `userId` que no existe). Detectado por reviewer durante audit del hotfix `afc5e4a` (vector colateral, fuera de scope). Sprint propio: o renombrar el campo a `userId`, o ajustar rule a `destinatarioId == request.auth.uid`. Priorizado **después** de C2 fase B App Check (post ventana 24-48h del `app_check_audit`).~~ **[RESUELTO en SPRINT-127 el 2026-05-10 — ruta B1 conservadora]** — Auditoría confirmó que TODOS los callers de `crearNotificacion` pasan `userId` (no `destinatarioId`). El renombrado masivo se hizo en commit `3733237` (sprint anterior). La rule ya gatea por `userId == auth.uid` desde antes. SPRINT-127 agregó cinturón+tirantes: (a) typing legacy `destinatarioId?` marcado más claro en `src/types/index.ts` con referencia a SPRINT-127; (b) `crearNotificacion()` emite `console.warn` runtime si llega `destinatarioId` o falta `userId`; (c) query dual del service se mantiene como red de seguridad (cleanup profundo B2 queda como sprint follow-up que requiere correr `auditoria-notis-legacy-todos.ts` para confirmar 0 docs huérfanos). Cazador P-007 ya bloquea reintroducir `destinatarioId` en callers nuevos.
- ~~**Gotcha — asunción frágil "para técnicos, `personal/{id}.id == auth.uid`".** Varios writes técnico-gateados en `ordenes_servicio` (rules de tecnico tipo `auth.uid == tecnicoId`) asumen que al alta de un técnico se crea el doc `personal/{auth.uid}` (NO `personal/{idAutogenerado}`). NO está enforced por código — depende de disciplina al alta manual desde `GestionUsuarios`. Si se rompe (ej: alta de técnico sin alinear ids), TODOS los writes técnico-gateados fallan con `permission-denied`. Antes de cazar bugs raros en flujos de técnico, verificar que su `personal/{id}` doc id matchea su `auth.uid`. Riesgo activo, no resuelto.~~ **[RESUELTO parcialmente en SPRINT-108 el 2026-05-07]** — la asunción quedó superada: a partir de SPRINT-105 (espejo `usuarios/{uid}`) y c4be345 (migración tecnicoId→uid), los writes técnico-gateados no dependen de que `personal.id == auth.uid`. El campo `tecnicoId` en `ordenes_servicio` ahora guarda el `auth.uid` (escrito desde dropdowns con `value={t.uid}`). El cazador P-006 (`check-tecnicoid-personal-id-misuse.ts`) detecta cualquier nuevo dropdown que reintroduzca el patrón. Lo que SIGUE siendo deuda: campos análogos (`operariaId`, `ayudanteId`, `responsableId`, `creadaPor`, etc.) tratados en SPRINT-111 pendiente.
...
**Sub-regla obligatoria — sprints que tocan `storage.rules` deben deployar antes de cerrar COMPLETADO.** Espejo de la sub-regla P-005 para `firestore.rules`. El comando es `npm run deploy:storage-rules` (corre `firebase deploy --only storage:rules` + actualiza `storage.rules.deployed.lock`). Si Jorge prefiere ejecutar el deploy manual (caso de SPRINT-138 inicial donde el baseline debe coincidir 1:1 con consola), el coordinator deja el sprint en estado "código commiteado, deploy pendiente de Jorge" y NO cierra COMPLETADO hasta que el lock se actualice. El cazador P-013 (`check-storage-rules-pendientes-deploy.ts`) bloquea pre-commit si hay diff entre repo y lock — la primera vez que se inicializa, emite WARN (no FAIL) hasta el primer deploy.
